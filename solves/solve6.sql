...
goal JOIN game ON (goal.teamid=game.id) 
WHERE goal.gtime<=10

--6.
--List the the dates of the matches and the name of the team 
--in which 'Fernando Santos' was the team1 coach.
SELECT mdate,teamname FROM eteam JOIN game ON (game.team1 = eteam.id)
...
WHERE teamId = (SELECT game.team1 WHERE game.team2 = 'GER') 
OR
teamId = (SELECT game.team2 WHERE game.team1 = 'GER')
--0r
SELECT DISTINCT player
FROM game JOIN goal ON matchid = id 
...
--11.
--For every match involving 'POL', show the 
--matchid, date and the number of goals scored.
SELECT matchid,mdate,COUNT(matchid)
  FROM game JOIN goal ON matchid = id 
 WHERE (team1 = 'POL' OR team2 = 'POL')GROUP BY matchid,mdate
 
--12.
--For every match where 'GER' scored, show 
--matchid, match date and the number of goals scored by 'GER'
SELECT matchid,mdate,COUNT(matchid)
  FROM game JOIN goal ON matchid = id 
