...
(actor.id = casting.actorid) WHERE 
...
WHERE 
id IN (SELECT actorid FROM movie JOIN casting 
...
WHERE ord = 1
AND movie.id IN
(SELECT movie.id FROM movie JOIN
casting ON (movie.id = casting.movieid) JOIN
actor ON (actor.id = casting.actorid) WHERE 
...
AND movieid IN(
SELECT movieid FROM casting JOIN actor 
